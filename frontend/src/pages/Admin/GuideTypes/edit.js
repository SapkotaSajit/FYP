...
function EditGuideTypes() {
  ...
  useEffect(() => {
    const fetchGuides = async () => {
      try {
        ...
        toast.error("Failed to load root guides catalog");
      }
    };
    fetchGuides();
  }, []);
    ...
    const fetchType = async () => {
      try {
        ...
        setFormData({
          ...
          guideTypes_image: type.guideTypes_image || null,
        });
        if (type.guideTypes_image) {
          setPreview(`${URL}${type.guideTypes_image}`);
        }
      } catch (error) {
        toast.error("Failed to retrieve classification details");
      }
    };
    fetchType();
  }, [id]);
  ...
  const handleChange = (e) => {
    ...
    // Standardizing file input handling to avoid previous inconsistencies
    if (name === "guide_image" && files[0]) {
      setFormData((prevState) => ({
        ...prevState,
        guideTypes_image: files[0],
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append("parent_id", formData.guide_id); // API seems to expect parent_id based on previous file

      if (formData.guideTypes_image instanceof File) {
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            ...
            Edit Guide Classification
          </h2>
          <p className="text-slate-500 text-sm font-medium">
            Update the metadata and organizational links for category #{id}.
          </p>
        </div>
      </div>
      ...
      <div className="glass rounded-[2rem] p-8 md:p-12 shadow-xl border border-slate-200/60 relative overflow-hidden group">
        ...
        <form onSubmit={handleSubmit} className="space-y-10 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-slate-700 flex items-center gap-2 ml-1">
                  <HiSelector className="text-blue-500" /> Root Association
                </label>
                <select
            ...
            <div className="space-y-6">
              <label className="text-sm font-bold text-slate-700 flex items-center gap-2 ml-1">
                <HiPhotograph className="text-indigo-500" /> Classification Icon
              </label>
              ...
              <div className="flex items-start gap-4 p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100 shadow-sm">
                ...
                <p className="text-[10px] text-indigo-700/80 font-bold leading-relaxed">
                  Refining classifications helps in better indexing of
                  instructional content for the end-user dashboard.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6">
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span className="font-bold tracking-wide">Sync Metadata</span>
                  <HiSave
