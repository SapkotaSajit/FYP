...
export const createService = async (req, res) => {
  try {
    let info = {
      service_image: req.file ? req.file.path : null,
      name: req.body.name,
  ...
  try {
    ...
    const servicesWithImage = await Promise.all(
      services.map(async (service) => {
        ...
        if (
          service.parent_id &&
          String(service.parent_id).trim() !== "null" &&
          String(service.parent_id).trim() !== ""
        ) {
          try {
            parentService = await Service.findByPk(service.parent_id);
          } catch (err) {
            console.error("Error fetching parent_id", service.parent_id, err);
          }
        }
        return {
          id: service.id,
