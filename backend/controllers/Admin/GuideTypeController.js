...
//     await guideTypes.save(); // Save the changes to the database
...
export async function getGuideTypesByGuideId (req, res)  {
  const guide_id = req.params.id; // Extract guide_id from request params
  try {
    const guideTypes = await GuideTypes.findAll({ where: { guide_id } });
    res.json(guideTypes);
  } catch (err) {
    console.error('Error fetching guide types by guide_id:', err);
